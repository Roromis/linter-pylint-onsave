...
module.exports =
  ...
  activate: ->
    require('atom-package-deps').install('linter-pylint-onsave')
    @subscriptions = new CompositeDisposable
    @subscriptions.add atom.config.observe 'linter-pylint-onsave.executable',
      (newExecutableValue) =>
        @executable = newExecutableValue
    @subscriptions.add atom.config.observe 'linter-pylint-onsave.rcFile',
      (newRcFileValue) =>
        @rcFile = newRcFileValue
    @subscriptions.add atom.config.observe 'linter-pylint-onsave.messageFormat',
      (newMessageFormatValue) =>
        @messageFormat = newMessageFormatValue
    @subscriptions.add atom.config.observe 'linter-pylint-onsave.pythonPath',
      (newPythonPathValue) =>
        @pythonPath = _.trim newPythonPathValue, path.delimiter
    @subscriptions.add atom.config.observe 'linter-pylint-onsave.workingDirectory',
      (newCwd) =>
    ...
    provider =
      ...
      lint: (activeEditor) =>
        ...
        env = Object.create process.env,
          PYTHONPATH:
            value: _.compact([process.env.PYTHONPATH, pythonPath]).join path.delimiter
            enumerable: true
